...
namespace MotionProfileMapper
{
    class Points
    {
        ...
        public float length() // length of the whole spline (roughly)
        {
            ...
            return (float) cumulativeDistances[cumulativeDistances.Count() - 1];
            
            // return (float)length(0, pts.Count() - 1);  // I think this is bad math, kept for reference.
        }
        ...
        public void fillCumulativeDist()
        {
    ...
    class Path
    {
        ...
        public int resolution = 40;  // 40 is pretty good for just over 10 ms of "time" value in generated log per point
        ...
        public void writeGen(String fileName)
        {
            ...
            using (System.IO.StreamWriter writetext = new System.IO.StreamWriter(fileName))
            {
                ...
                for (int i = 0; i < leftTrack.pts.Count(); i++)
                {
                    ...
                    {
                        writetext.WriteLine("       {" + leftTrack.cumulativeDistances[i] + ", " + leftTrack.defVelMap[i] + ", " + leftTrack.defTimeMap[i] + "}};");
                        leftTrack.sumMs += leftTrack.defTimeMap[i];
                    }
                    else
                    {
                        writetext.WriteLine("       {" + leftTrack.cumulativeDistances[i] + ", " + leftTrack.defVelMap[i] + ", " + leftTrack.defTimeMap[i] + "},");
                ...
                for (int i = 0; i < rightTrack.pts.Count(); i++)
                {
                    ...
                    {
                        writetext.WriteLine("       {" + rightTrack.cumulativeDistances[i] + ", " + rightTrack.defVelMap[i] + ", " + rightTrack.defTimeMap[i] + "}};");
                        rightTrack.sumMs += rightTrack.defTimeMap[i];
                    }
                    else
                    {
                        writetext.WriteLine("       {" + rightTrack.cumulativeDistances[i] + ", " + rightTrack.defVelMap[i] + ", " + rightTrack.defTimeMap[i] + "},");
